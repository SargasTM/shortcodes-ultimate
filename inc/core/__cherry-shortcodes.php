...
class Su_Shortcodes {
	...
	public static function row( $atts = null, $content = null ) {
		$atts = shortcode_atts( MPCEShortcode::addStyleAtts(array(
			'type'  => 'fixed-width',
			'class' => '',
		)), $atts, 'row' );
                $prefix = su_cmpt();
		...
		return '<div class="' . esc_attr( $container ) . su_ecssc( $atts ) . MPCEShortcode::getMarginClasses($margin) . MPCEShortcode::getBasicClasses($prefix . 'row', true) . $mp_style_classes . '"><div class="row">' . do_shortcode( $content ) . '</div></div>';
	}
	...
	public static function row_inner( $atts = null, $content = null ) {
		...
		return '<div class="row' . su_ecssc( $atts ) . MPCEShortcode::getMarginClasses($margin) . MPCEShortcode::getBasicClasses($prefix . 'row_inner', true) . $mp_style_classes . '">' . do_shortcode( $content ) . '</div>';
	}
	...
	public static function col( $atts = null, $content = null ) {
		...
		if (!empty($mp_style_classes)) $mp_style_classes = ' ' . $mp_style_classes;
		$prefix = su_cmpt();
		$class .= MPCEShortcode::getMarginClasses($margin) . MPCEShortcode::getBasicClasses($prefix . 'col', true) . $mp_style_classes;

		return '<div class="' . trim( esc_attr( $class ) ) . '" ' . $style . '>' . do_shortcode( $content ) . '</div>';
	}
	...
	public static function col_inner( $atts = null, $content = null ) {
		...
		if (!empty($mp_style_classes)) $mp_style_classes = ' ' . $mp_style_classes;
		$prefix = su_cmpt();
		$class .= MPCEShortcode::getMarginClasses($margin) . MPCEShortcode::getBasicClasses($prefix . 'col_inner', true) . $mp_style_classes;
